...
annotation5 <- data.frame(
   x = c(2010),
   y = c(-4000000),
   label = c("10k meters sq =")
)
annotation6 <- data.frame(
   x = c(2010),
   y = c(-3500000),
   label = c("about 2.5 football feilds")
)
...
forest %>% 
  ...
  labs(title = "Net forest loss by country", x = NULL, y = "Hectares") +
  scale_color_manual(values = c("#009739")) +
  scale_x_continuous(breaks = seq(from = 1990, to = 2015, by = 5)) +
  scale_y_reverse(labels = unit_format(unit = "M", scale = -1e-6)) +
...
---
### Is this change proportional to brazil's deforestation development over time?
...
.pull-left[
```{r compare, echo = FALSE, message = FALSE, out.width = "500%", out.height= "500%"}
data2 %>%
  filter(year >= 1977) %>%
...
.pull-right[
```{r observe, echo = F, message = F, out.width = "200%"}
forest2 <- forest %>%
  filter(entity == "World")
...
#### The net loss in global forest area to use land for another purpose (i.e. creating pasture for cattle) matches the pattern of increase in soybean use shared by animals.
...
- Increased global use of soybeans is correlated with increased deforestation in Brazil, the world's leading soybean producer (surpassing USA as of May 2021)

- Increased proportion of soybeans used for animal feed (as compared to human) is correlated with continued land conversion (natural land --> human used), albeit at a slower rate, potentially for increased livestock rearing. 
