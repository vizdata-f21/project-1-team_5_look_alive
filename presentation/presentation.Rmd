...
forest %>% 
  ...
  labs(title = "Net Forest Conversion (hectares)", x = NULL, y = NULL) +
  scale_color_manual(values = c("#009739")) +
  scale_x_continuous(breaks = seq(from = 1990, to = 2015, by = 4))
```
...
```{r cumulative_brazil_only, echo=F, fig.alt="Brazil lost 30m hectares of forest from 2000 to 2013."}
...
brazil_loss %>% select(year, total_brazil_forest_loss_hectares) %>%
  ...
  geom_hline(aes(yintercept = 0), color = "red", size = 2) +
  labs(title = "Brazilian Forest Loss (hectares)", subtitle = "2000 to 2013", x = NULL, y = NULL) +
  scale_y_continuous(labels = label_number_si()) +
  scale_x_continuous() +
...
---
## Causes of Brazil's Loss of Forest
...
brazil_loss_no_total <- brazil_loss_no_total %>%
  ...
  factor(levels=c("Other", 'Pasture', 'Fire', "Logging", "Small-Scale Farming", "Commercial Crops"))) %>%
  group_by(cause, year) %>%
...
# Question Two
### How does the proportion of soybean production used for human food versus for feeding animal change over time? Is this change in proportions related to global deforestation development over time?
